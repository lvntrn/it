class Player4
{
  ...
  Player4 (float _x, float _y)
  {
    this.x= _x;
    this.y= _y;

    this.frameRun = 1;
    this.imagePrefix = "schiffsjunge_";
  ...
  void winPlayer(){
   textSize(32);
   text("player 4 hat gewonnen",10,30);
   fill(0,0,0);
  }
  ...
  void jump1() 
  {
    ...
    if ( (jump == false)  )  
    {
      y+= 10;//player geht wieder auf den Boden zurück
    }
    if (ground4 == false && jump == false && y >= 400)
    {
      ground4 = true;
    }
  }
  ...
  void animation()
  {
    if (ground4 == true && this.right) //wenn pferd auf dem Boden und rechtspfeil
    {
      image(images[frame], this.x, this.y);
    }
    if (this.ground4 == true)//wenn Pferd auf boden
    {
      image(images[frame], this.x, this.y);
    }
