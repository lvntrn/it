class Player2
{
  ...
  Player2 (float _x, float _y)
  {
    this.x= _x;
    this.y= _y;

    this.frameRun = 1;
    this.imagePrefix = "seefahrer_";
  ...
  void jump1() 
  {
    ...
    if (ground2 == false && jump == false && y >= 400)
    {
      ground2 = true;
    }
  }
  ...
  void animation()
  {
    if (ground2 == true && this.right) //wenn pferd auf dem Boden und rechtspfeil
    {
      image(images[frame], this.x, this.y);
    }
    if (this.ground2 == true)//wenn Pferd auf boden
    {
      image(images[frame], this.x, this.y);
    }
